...
Sanskrit.prototype = {
  ...
  htmlize: function(textile, escape){
    ...
    for (var i=0; i<paragraphs.length; i++) {
      ...
      paragraphs[i] = paragraphs[i].replace(/^.*?(h4).*?((?:[a-z][a-z0-9_]*))/gi, '<h4>$2</h4>');
      paragraphs[i] = paragraphs[i].replace(/^.*?(h5).*?((?:[a-z][a-z0-9_]*))/gi, '<h5>$2</h5>');
      paragraphs[i] = paragraphs[i].replace(/^.*?(h6).*?((?:[a-z][a-z0-9_]*))/gi, '<h6>$2</h6>');
